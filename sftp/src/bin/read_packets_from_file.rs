...
fn main() {
    ...
    for (i, line) in reader.lines().enumerate() {
        match line {
            Ok(content) => {
                let num = content
                    .parse::<u8>()
                    .expect(format!("Failed to parse line {i} as u8").as_str());
                bytes.push(num);
            }
    ...
    while source.remaining() > 0 {
        match SftpPacket::decode(&mut source) {
            Ok(packet) => match packet {
                SftpPacket::Name(req_id, name) => {
                    ...
                    for i in 0..name.count {
                        println!(
                            "--({i}) Entry: {:?}",
                            NameEntry::dec(&mut source)
                                .expect("Failed to decode NameEntry")
                        );
                    }
                }
            ...
            Err(e) => {
                println!(
                    "Error decoding packet: {:?}. Up to: {:?}",
                    e,
                    source.buffer_used().len()
                );
        ...
        println!(
            "Last 9 bytes : {:?}, Lines {:?}-{used}, Counters: ({last_used}/{used}) [last/total decoded]\n",
            &source.buffer_used()[used - 9..],
            prev_used + 1
        );
    }
}
